...
11. [x] **선수별 A4 1페이지 공식 전력분석 리포트 & PDF 인쇄 엔진 구축 (`/report/player/<id>`, `/report/all`)**:
   ...
   - **리포트 구성 요소**:
     - 상단 W.B.C 공식 엠블럼 및 기술기록위원회 공인 발행 표기
     - 선수 프로필, 소속팀, 타자 스타일 타이틀(슈퍼 슬러거, 클러치 히터, 정밀 타격 마스터 등)
     - 8대 누적 KPI (타율, 타점, 홈런, OPS, 출루율, 장타율, 안타, 평균 활약점수)
     - 5대 능력치 레이더 차트 + 경기별 타율 성장 궤적 라인 차트 병렬 배치
     - 커리어 하이(최고 활약 경기) 및 영예의 수상(MVP/MIP/언성히어로) 횟수 배지
     - 전 경기 상세 출전 타격 로그 (Game-by-Game)
   - **2가지 모드 원클릭 지원**:
