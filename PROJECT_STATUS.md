...
4. [x] **프론트엔드 UI/UX 구현 (`templates/`, `static/`)**:
   ...
   - **SNS 공유 메시지 생성기 (`sns.html`) — 타점 기반 MVP 추천 시스템 구축**:
     - **해결사/클러치 가중치 산정식**: `(타점 * 2.5) + (안타 * 1.5) + (홈런 * 2.0) + (타율 * 1.0)`으로 점수를 만들어낸 선수를 1순위 MVP로 자동 추천
     - MVP 후보 선택창에 각 선수의 **타점(RBI)**을 금색 배지로 시각화하고 타점왕 하이라이트 박스 추가
     - SNS 메시지(장문/단문)에 MVP의 타수, 안타, 타율과 함께 **타점 기록**(`(X타수 Y안타 Z타점, 타율 0.XXX)`)을 필수로 포함
